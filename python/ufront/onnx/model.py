...
class ONNXModel(object):
    ...
    def handleAdd(self, node, node_to_output):
        ...
        if type(input0) == Tensor and type(input1) == Tensor:
            ...
            if type(input1) in [
                ...
                return self.umodel.sadd(input=input0, scalar=ss, name=node.name)
            else:
            ...
            if type(input0) in [
                ...
                return self.umodel.sadd(input=input1, scalar=ss, name=node.name)
            else:
                ...
                return self.umodel.sadd(x=input0, y=input1, name=node.name)
        else:
    ...
    def handleSub(self, node, node_to_output):
        ...
        if type(input0) == Tensor and type(input1) == Tensor:
            ...
            if type(input1) in [
                ...
                return self.umodel.ssub(
                    input=input0,
                    scalar=input1,
                    scalar_position='"RIGHT"',
                    name=node.name,
                )
            else:
                assert type(input1) == np.ndarray, "The given input is not an ndarray!"
    ...
    def handleMul(self, node, node_to_output):
        ...
        if type(input0) == Tensor and type(input1) == Tensor:
            ...
            if type(input0) in [
                ...
                return self.umodel.multiply(x=input1, y=input0, name=node.name)
        else:
            return input0 * input1
    ...
    def handleConcat(self, node, node_to_output):
        ...
        for i in range(len(inputs)):
            ...
            if type(input).__name__ == "ValueInfoProto":
                ...
                inputs[i] = self.umodel.create_tensor(
                    dims, DataType.Float, True, input.name
                )
            elif type(input) == np.ndarray:
                param_op = self.umodel.parameter(
                    np_tensor=input,
                    dtype=numpy_to_ufront_dtype(input.dtype),
                    requires_grad=True,
                    name=node.input[i],
                )
                ...
                allTensors = False
        if allTensors:
            return self.umodel.concat(
                tensors=inputs, axis=attribute["axis"].i, name=node.name
            )  # tensor concat
        else:
            ret = []
            for input in inputs:
    ...
    def handleSplit(self, node, node_to_output):
        ...
        if type(input) == list:
            return input
        return self.umodel.split(
            input=input, sizes=list(split), axis=axis, name=node.name
        )
    ...
    def handleGlobalAveragePool(self, node, node_to_output):
        ...
        return self.umodel.pool2d(
            input=input,
            output_size=[1, 1] if len(input.shape) > 3 else [1],
            stride=[1, 1] if len(input.shape) > 3 else [1],
            pad=[0, 0] if len(input.shape) > 3 else [0],
            pool_type=PoolType.POOL_ADAPTIVE,
            name=node.name,
        )
    ...
    def handleGlobalMaxPool(self, node, node_to_output):
        ...
        return self.umodel.pool2d(
            ...
            stride=[1, 1] if len(input.shape) > 3 else [1],
            pad=[0, 0] if len(input.shape) > 3 else [0],
            pool_type=PoolType.POOL_ADAPTIVE_MAX,
            name=node.name,
        )
    ...
    def handleBatchNormalization(self, node, node_to_output):
        ...
        if training_mode and len(node.input) > 4:
            ...
            weight_op = self.umodel.parameter(
                np_tensor=weight,
            ...
            running_mean_op = self.umodel.parameter(
            ...
            return self.umodel.batch_norm(
                ...
                momentum=(
                    momentum[0]
                    if type(momentum) == list or type(momentum) == tuple
                    else momentum
                ),
                track_running_stats=training_mode,
            )
        else:
            return self.umodel.batch_norm(
                input=input,
    ...
    def handleSqueeze(self, node, node_to_output):
        ...
        axis = (
            axes[0] - 1
            if type(axes) == tuple or type(axes) == list or type(axes) == np.ndarray
            else int(axes)
        )
        if axis < 0:
            axis = 0
    ...
    def handleNeg(self, node, node_to_output):
        input = node_to_output[node.input[0]]
        return self.umodel.smultiply(input=input, scalar=-1.0, name=node.name)
    ...
    def handleGelu(self, node, node_to_output):
        ...
        return self.umodel.gelu(
            input=input1 if type(input1) == Tensor else input2,
            approximate=approximate != 0.5,
            name=node.name,
        )
    ...
    def unpack_rawdata(self, raw_data, data_type, shape, name=None):
        ...
        output = np.array(
            struct.unpack(fmt, raw_data), dtype=ufront_to_numpy_dtype(data_type)
        )
        ...
        if len(shape) == 0 or (len(shape) == 1 and shape[0] == 1):
            ...
            if name != None and (
                name == "class_token" or name == "encoder.pos_embedding"
            ):
                weight_op = self.umodel.parameter(
                    np_tensor=output,
                    dtype=numpy_to_ufront_dtype(output.dtype),
                    requires_grad=True,
                    name=name,
                )
    ...
    def handleConstant(self, node, node_to_output):
        ...
        if len(tensor.dims) > 1:
            ...
            output = self.addTensor(
                np_tensor, False, "constant_tensor" + str(ONNXModel.const_tensor_idx)
            )
    ...
    def handleMatMul(self, node, node_to_output):
        ...
        if type(input2) != Tensor:
            ...
            weight_op = self.umodel.parameter(
                np_tensor=input2.astype(np.float32),
                dtype=numpy_to_ufront_dtype(input2.dtype),
                requires_grad=True,
                name=node.input[1],
            )
            # self.operators.append(operator)
    ...
    def addTensor(self, np_tensor, requires_grad, name):
        operator = self.umodel.parameter(
            np_tensor=np_tensor,
    ...
    def handleRandomUniformLike(self, node, node_to_output):
        ...
        return self.umodel.uniform_like(
            input=operator.get_output(0),
            dtype=numpy_to_ufront_dtype(np_tensor.dtype),
            name=node.name,
        )
    ...
    def handleDiv(self, node, node_to_output):
        ...
        if type(scalar) in [
            ...
            return self.umodel.divide(
                input=input1, operand=scalar.tolist(), name=node.name
            )
    ...
    def handleSlice(self, node, node_to_output):
        ...
        if type(axis) == np.ndarray:
            ...
            return self.umodel.slice_tensor(
                input=input,
    ...
    def handleGather(self, node, node_to_output):
        ...
        attribute = {x.name: x for x in node.attribute}
        axis = attribute["axis"].i
        if type(axis) == int and (
            type(start) == int or type(start) == np.int32 or type(start) == np.int64
        ):
            output_shape = input.shape
    ...
    def handleLayerNormalization(self, node, node_to_output):
        ...
        return self.umodel.layer_norm(
            input=input_tensor,
            normalized_shape=list(weight_shape),
            eps=eps,
    ...
    def handleMultiHeadAttention(self, node, node_to_output):
        ...
        operator_q = self.umodel.parameter(
            np_tensor=weight_q,
            dtype=numpy_to_ufront_dtype(weight_q.dtype),
            requires_grad=True,
            name=node.name + "_weight_q",
        )
        operator_k = self.umodel.parameter(
            np_tensor=weight_k,
            dtype=numpy_to_ufront_dtype(weight_k.dtype),
            requires_grad=True,
            name=node.name + "_weight_k",
        )
        operator_v = self.umodel.parameter(
            np_tensor=weight_v,
            dtype=numpy_to_ufront_dtype(weight_v.dtype),
            requires_grad=True,
            name=node.name + "_weight_v",
        )
        operator_o = self.umodel.parameter(
            np_tensor=weight_o,
            dtype=numpy_to_ufront_dtype(weight_o.dtype),
            requires_grad=True,
            name=node.name + "_weight_o",
        )
    ...
    def handleClip(self, node, node_to_output):
        ...
        return self.umodel.clip(
            input=input, minimum=minimum, maximum=maximum, name=node.name
        )
    ...
    def process_initializer(self, inputs, initializer):
        ...
        for item in initializer:
            ...
            if len(item.raw_data) > 0:
                values = self.unpack_rawdata(
                    item.raw_data, datatype, item.dims, item.name
                )
            else:
    ...
    def apply(self, input_tensors):
        ...
        for i in range(len(self.model.graph.input)):
            ...
            if (
                input.name.find("input") >= 0
                or (i == 0 and input.name.find("x") >= 0)
                or input.name.find("onnx::") >= 0
            ):
                input_tensor = (
                    input_tensors[i]
                    if type(input_tensors) == list
                    else input_tensors[input.name]
                )
                if type(input_tensor) != Tensor:
                    dtype = input_tensor.dtype
                    if (
                        type(input_tensor) != np.ndarray
                        and input_tensor.__module__ == "torch"
                    ):
                        dtype = input_tensor.numpy().dtype
                    input1 = np.ones(shape=input_tensor.shape, dtype=dtype)
                    input1[:] = input_tensor
                    inputname = input.name.replace(
                        "::", ""
                    )  # "::" not valid in MLIR for name
                    input_tensor = Tensor(
                        np_tensor=input1,
                ...
                input.name.find(".weight") < 0
                and input.name.find(".bias") < 0
                and input.name.find("onnx::") < 0
            ):
                self.inputs[input.name] = self.convert_to_tensor(input)
                if input.name == "class_token" or input.name == "encoder.pos_embedding":
                    operator = self.addTensor(
                        self.inputs[input.name].ndarray, True, input.name
                    )
            ...
            for node in self.model.graph.node:
                if (
                    ...
                    for add_node in self.model.graph.node:
                        if add_node.op_type == "Add" and (
                            add_node.input[0] == output or add_node.input[1] == output
                        ):
                            # print(node, add_node)
                            flag_found = True
    ...
    def _fusion_layer(self, name):
        ...
        while True:
            ...
            for node in self.model.graph.node:
                if node.name.find("/" + name) >= 0 and (
                    not stop_node or node.name.find(stop_node) == -1
                ):  # for not standard fusion pattern

                    if (
                        name == "attention" and node.name.find("LayerNorm") > 0
                    ):  # do not fuse LayerNorm into multihead_attention
                        idx += 1
                        continue
                    else:
                        flag_found = True
                        ...
                        if (
                            name == "embeddings"
                            and node.name.find("/embedding_lookup") >= 0
                            and len(target_input) < 2
                        ):
                        ...
                        if name in self.ATTENTION_PATTERN and node.op_type == "MatMul":
                            for inp in node.input:
                                if inp in self.inputs.keys():
                                    if (
                                        type(self.inputs[inp]) == np.ndarray
                                        and inp not in target_input
                                    ):
                                        target_input.append(
                                            inp
                                        )  # weights for multihead_attention
                    ...
                    if is_target:
                        ...
                        node = onnx.helper.make_node(
                            ...
                            name="Ext" + name + "_" + str(target_idx),
                        )
...
class ONNXModelKeras(ONNXModel):
    def __init__(
        ...
        super(ONNXModelKeras, self).__init__(
        ...
        for node in onnx_model.graph.node:
            if node.name.find("u_front_keras/") != -1:
                node.name = node.name[
                    node.name.find("keras_model/") + len("u_front_keras/") + 1 :
                ]
            # print(node.name)
        for initializer in self.model.graph.initializer:
            if (
                "/bias" in initializer.name
                or "/BiasAdd/ReadVariableOp" in initializer.name
            ) and "dense" in initializer.name:
    ...
    def _create_initializer_tensor(self, ffconfig, input):
        if len(input.dims) == 1:
            ...
            assert 0
        tensor = self.umodel.create_tensor(
            dims,
            DataType.Float,
            True,
            "constant_tensor" + str(ONNXModel.const_tensor_idx),
        )
        ONNXModel.const_tensor_idx += 1
        return tensor
...
class UFrontONNX(ONNXModel):
    def __init__(
        ...
        super(UFrontONNX, self).__init__(
            onnx_model, self.umodel, simplify, pass_weights, transformer=transformer
        )
        ...
        self.transformer = transformer
        self.umodel.optimizer = Optimizer(
            params={
                "type": "sgd",
                "lr": "0.01",
                "momentum": "0",
