...
try:
    ...
    print(
        "You need to first install tensorflow-cpu and tf2onnx before using keras models!"
    )
...
class BaseModel(object):
    def __init__(
        self, inputs, onnx_model, batch_size, simplify, transformer, pass_weights
    ):
        ...
        self._my_onnx_model = ONNXModelKeras(
            self._onnx_model,
            self.ufront_model,
            simplify,
            self.transformer,
        ...
        for key, input in inputs.items():
            ...
            input1[:] = input
            input_tensor = Tensor(
                np_tensor=input1, dtype=numpy_to_ufront_dtype(input1.dtype), name=key
            ...
            if isinstance(optimizer, tf_keras_optimizer.SGD) == True:
                self._ffoptimizer = Optimizer(
                    params={
                        ...
                        "momentum": str(optimizer.momentum.numpy()),
                        "nesterov": str(optimizer.nesterov),
                        "weight_decay": str(optimizer.decay.numpy()),
                    }
                )
                ...
                self._ffoptimizer = Optimizer(
                    params={
                        "type": "adam",
                        "lr": str(optimizer.learning_rate.numpy()),
                        "beta_1": str(optimizer.beta_1.numpy()),
                        "beta_2": str(optimizer.beta_2.numpy()),
                        "epsilon": str(optimizer.epsilon.numpy()),
                    }
                )
            else:
    ...
    def _create_data_loaders(self, x_trains, y_train):
        ...
        for x_train in x_trains:
            dataloader = self.ufront_model.create_data_loader(
                self._input_tensors[idx].ffhandle, x_train
            )
            self._input_dataloaders.append(dataloader)
            self._input_dataloaders_dim.append(len(input_shape))
            idx += 1
        dataloader = self.ufront_model.create_data_loader(
            self._label_tensor.ffhandle, y_train
        )
...
class UFrontKeras(tf_keras_Model):
    def __init__(
        ...
        super(UFrontKeras, self).__init__(
            inputs=base_model.inputs, outputs=base_model.output, name=base_model.name
        )
        if isinstance(inputs, list) == False:
            assert (
                0
            ), "Inputs must be in list format, e.g., [input_tensor1, input_tensor2]"
        input_dict = {}
        for i in range(len(base_model.inputs)):
            input = base_model.inputs[i]
            input_dict[input.name] = inputs[i]
        self.onnx_model = tf2onnx.convert.from_keras(
            self, opset=18 if transformer else 17
        )
        self._base_model = BaseModel(
            inputs=input_dict,
            onnx_model=self.onnx_model[0],
